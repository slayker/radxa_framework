...
status_t HWComposer::prepare() {
    ...
    // use gpu repaint the last frame to reduce bandwith.
        ...
        if (disp.list) {
            ...
            for (size_t i=0 ; i<disp.list->numHwLayers ; i++) {
                ...
                if ( l.compositionType == HWC_OVERLAY ||
                     l.compositionType == HWC_TOWIN0 || l.compositionType == HWC_TOWIN1 )
                {
                    NeedRepaint = true;
                }
                if (l.bufferUpdate) {
                    l.bufferUpdate = 0;
                    // back to gpu composer to reduce bandwidth when small status bar update only.
...
public:
    ...
    virtual buffer_handle_t getHandle() const {
        return getLayer()->handle;
    }
    virtual uint32_t getFlags() const {
        return getLayer()->flags;
    }

    hwc_layer_1_t * gethwcLayer() const {
        return ( hwc_layer_1_t * )getLayer();
    }
    virtual char  * getLayerName()  {
    ...
    virtual void setLayerupdate(int32_t update) {
        getLayer()->bufferUpdate = update;
    }
    virtual void setLayercount(int32_t count) {
        getLayer()->bufferCount = count;
    }
    virtual void setLayerchange(int32_t change) {
        getLayer()->bufferChange = change;
    }
...
uint32_t HWComposer::getLayerNum (int32_t id) {

    if (uint32_t(id) <= 31 && mAllocatedDisplayIDs.hasBit(id)) {
        const DisplayData& disp(mDisplayData[id]);
        if (mHwc && disp.list) {
            return disp.list->numHwLayers;
        }
    }
    return 0;
}
