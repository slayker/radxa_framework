...
void LayerBase::initStates(uint32_t w, uint32_t h, uint32_t flags)
{
    ...
    if (!mFlinger->mUseLcdcComposer || (flags & ISurfaceComposerClient::eFakeTransformation) == 0)
...
bool LayerBase::setPosition(float x, float y) {
    ...
    if (mShouldTransform) {
        float convertX = x;
        float convertY = y;
        float rw = (float)mCurrentState.requested.w;
        float rh = (float)mCurrentState.requested.h;
        float rcos = mCurrentState.transform.rcos();
        float rsin = mCurrentState.transform.rsin();
        sp<const DisplayDevice> hw(mFlinger->getDefaultDisplayDevice());
        switch (hw->getHardwareRotation()) {
        case DisplayState::eOrientation270:
            x = (int)(convertY - rh * rsin);
            y = (int)((float)hw->getHeight() - rh * rcos - convertX);
            break;
        case DisplayState::eOrientation90:
            x = (int)((float)hw->getWidth() - rw * rcos - convertY);
            y = (int)(convertX + rw * rsin);
            break;
        case DisplayState::eOrientation180:
            x = (int)((float)hw->getWidth() - convertX - rw * rcos - rh * rsin);
            y = (int)((float)hw->getHeight() - convertY - rh * rcos + rw * rsin);
            break;
        }
    }
...
bool LayerBase::setSize(uint32_t w, uint32_t h) {
    sp<const DisplayDevice> hw(mFlinger->getDefaultDisplayDevice());
    if (mShouldTransform && hw->getHardwareRotation() % 2 == 1) {
        int tmp = w;
        w = h;
        h = tmp;
	}
...
bool LayerBase::setCrop(const Rect& crop) {
	Rect convertCrop(crop.width(), crop.height());
    convertCrop.offsetTo(crop.left, crop.top);
    if (mShouldTransform) {
        sp<const DisplayDevice> hw(mFlinger->getDefaultDisplayDevice());
        Point rb;
        float rw = (float)mCurrentState.requested.w;
        ...
        switch (hw->getHardwareRotation()) {
        case DisplayState::eOrientation270:
            convertCrop.offsetTo(crop.top, rh - crop.left - crop.width());
            rb.x = crop.top+crop.height();
            rb.y = rh - crop.left;
            convertCrop.setRightBottom(rb);
            break;
        case DisplayState::eOrientation90:
            convertCrop.offsetTo(rw-crop.top-crop.height(), crop.left);
            rb.x = rw-crop.top;
            rb.y = crop.left+crop.width();
            convertCrop.setRightBottom(rb);
            break;
        case DisplayState::eOrientation180:
            convertCrop.offsetTo(rw - crop.left - crop.width(), rh - crop.top - crop.height());
            rb.x = rw - crop.left;
            rb.y = rh - crop.top;
            convertCrop.setRightBottom(rb);
            break;
    ...
    if (mCurrentState.requested.crop == convertCrop)
        return false;
    mCurrentState.sequence++;
    mCurrentState.requested.crop = convertCrop;
    requestTransaction();
    return true;
}
...
void LayerBase::computeGeometry(const sp<const DisplayDevice>& hw, LayerMesh* mesh) const
{
    ...
    const Transform tr(hw->getTransform(mShouldTransform) * s.transform);
...
void LayerBase:: ReleaseOldBuffer(){
}
        ...
        HWComposer::HWCLayerInterface& layer)
{
    ...
    const Transform& tr(hw->getTransform(mShouldTransform));
...
void LayerBase::setPerFrameData(const sp<const DisplayDevice>& hw,
    ...
    if (mFlinger->mUseLcdcComposer && mQueuedFramesOld > 0) {
        mQueuedFramesOld --;
        layer.setLayerupdate(1);
    }
}
...
void LayerBase::dump(String8& result, char* buffer, size_t SIZE) const
{
    ...
    snprintf(buffer, SIZE,
            ...
            "alpha=0x%02x, flags=0x%08x, mShouldTransform=%1d, tr=[%.2f, %.2f][%.2f, %.2f]\n",
            s.layerStack, s.z, s.transform.tx(), s.transform.ty(), s.active.w, s.active.h,
            s.active.crop.left, s.active.crop.top,
            s.active.crop.right, s.active.crop.bottom,
            isOpaque(), needsDithering(), contentDirty,
            s.alpha, s.flags, mShouldTransform,
