...
void Layer::onFrameQueued() {
    ...
    if (mFlinger->mUseLcdcComposer) {
        mQueuedFramesOld = mQueuedFrames;
    }
}
...
void Layer::setName(const String8& name) {
    ...
    if (mFlinger->mUseLcdcComposer && strcmp("com.android.systemui.ImageWallpaper", name.string())) {
        const sp<const DisplayDevice> hw(mFlinger->getDefaultDisplayDevice());
        uint32_t realtransform = (hw->getTransform(false)).getOrientation() | 0x08;
        realtransform = (realtransform << 24) & GRALLOC_USAGE_ROT_MASK;
        mSurfaceTexture->setConsumerUsageBits(getEffectiveUsage(realtransform));
    }
}
...
#ifdef TARGET_RK30
int Layer::getBufferState(uint32_t addr)
{
    return mSurfaceTexture->getBufferState(addr);
}
#endif
        ...
        HWComposer::HWCLayerInterface& layer)
{
    ...
    const State& s(drawingState());
    if (s.alpha < 0xFF) {
        layer.setSkip(true);   // remove for a short time ,for androdi4.1 video optimize hadnot been merge
    }
    ...
    const Transform tr(hw->getTransform(mShouldTransform) * s.transform * bufferOrientation);
    ...
    if (finalTransform & Transform::ROT_INVALID) {
        ...
        layer.setRealTransform(realtransform);
    ...
    if (mFlinger->mUseLcdcComposer) {
        ...
        int32_t bufferChange = 0;
        mSurfaceTexture->mBufferQueue->query(1001, &bufferCount);
        layer.setLayercount(bufferCount);
        mSurfaceTexture->mBufferQueue->query(1002, &bufferChange);
        layer.setLayerchange(bufferChange);
    }
}
...
void Layer::onDraw(const sp<const DisplayDevice>& hw, const Region& clip) const
{
    ...
    if (CC_UNLIKELY(mActiveBuffer == 0)) {
        ...
        for (size_t i=0 ; i<count ; ++i) {
            ...
            if (layer.get() == static_cast<LayerBase const*>(this))
                break;
            under.orSelf( hw->getTransform(mShouldTransform).transform(layer->visibleRegion) );
        }
    ...
    if (!blackOutLayer) {
        ...
        const bool useFiltering = true;//getFiltering() || needsFiltering(hw) || isFixedSize();
        //const bool useFiltering = true;//getFiltering() || needsFiltering(hw) || isFixedSize();
...
Region Layer::latchBuffer(bool& recomputeVisibleRegions)
{
    ...
    if (mQueuedFrames > 0) {
        ...
        if (recomputeVisibleRegions) {
            LayerBase::setPosition();
            LayerBase::commitTransaction();
        }
...
{
    if (mFlinger->mUseLcdcComposer) {
        mSurfaceTexture->ReleaseOldBuffer();
    }
}
...
void Layer::updateTransformHint(const sp<const DisplayDevice>& hw) const {
    ...
    if (!mFlinger->mDebugDisableTransformHint) {
        ...
        const Transform& planeTransform(hw->getTransform(mShouldTransform));
