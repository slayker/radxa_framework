...
void SurfaceFlinger::bootFinished()
{
    ...
    for (size_t i=0 ; i<count ; i++) {
        ...
        if (!strcmp(layer->getName().string(), "BootAnimation")) {
            removeLayer(layer);
        }
    }
...
EGLConfig SurfaceFlinger::selectEGLConfig(EGLDisplay display, EGLint nativeVisualId, bool useLcdcComposer) {
    ...
    if (useLcdcComposer)
        err = selectConfigForAttribute(display, attribs, nativeVisualId, HAL_PIXEL_FORMAT_RGB_565, &config);
    else
...
status_t SurfaceFlinger::readyToRun()
{
    ...
    mEGLConfig  = selectEGLConfig(mEGLDisplay, format, mUseLcdcComposer);
    ...
    for (size_t i=0 ; i<DisplayDevice::NUM_DISPLAY_TYPES ; i++) {
        ...
        if (mHwc->isConnected(i) || type==DisplayDevice::DISPLAY_PRIMARY) {
            ...
            sp<DisplayDevice> hw = new DisplayDevice(this,
                    type, isSecure, token, stc, fbs, mEGLConfig, mHardwareOrientation);
    ...
    startBootAnim();
	property_set("sys.gsflg.version","1.000"); // add version
    property_set("sys.yuv.rgb.format","1");
    char property[PROPERTY_VALUE_MAX];
    if (property_get("video.use.overlay", property, NULL) && atoi(property) > 0) {
...
void SurfaceFlinger::rebuildLayerStacks() {
    ...
    if (CC_UNLIKELY(mVisibleRegionsDirty)) {
        ...
        for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
            ...
            const Transform& realTR = hw->getRealTransform();
            const Rect bounds(hw->getBounds());
            if (hw->canDraw()) {
                SurfaceFlinger::computeVisibleRegions(currentLayers,
                        hw->getLayerStack(), dirtyRegion, opaqueRegion,
                        mUseLcdcComposer && hw->getHardwareRotation() == 1);
                ...
                for (size_t i=0 ; i<count ; i++) {
                    ...
                    if (s.layerStack == hw->getLayerStack()) {
                        Region drawRegion(layer->mShouldTransform ?
                                             tr.transform(layer->visibleNonTransparentRegion) :
                                             realTR.transform(layer->visibleNonTransparentRegion));
...
void SurfaceFlinger::setUpHWComposer() {
    ...
    if (hwc.initCheck() == NO_ERROR) {
        ...
        if (mUseLcdcComposer) {
            // release old buffer when gles composition
            for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
                sp<const DisplayDevice> hw(mDisplays[dpy]);
                const int32_t id = hw->getHwcDisplayId();
                if (id >= 0) {
...
static int gfd_rga = -1;

int SurfaceFlinger::RenderToWfd(Target_info *dst,Target_info *src)
{
    ...
    Rga_Request.src.yrgb_addr =  (int)src->VAddr ;
    Rga_Request.src.vir_w = src->Width;
    Rga_Request.src.vir_h = src->Height;
	...
	Rga_Request.dst.yrgb_addr =(int)dst->VAddr;
   	Rga_Request.dst.vir_w = dst->Width;
    Rga_Request.dst.vir_h = dst->Height;
	Rga_Request.dst.act_w = Rga_Request.dst.vir_w;
	Rga_Request.dst.act_h = Rga_Request.dst.vir_h;

    Rga_Request.dst.format = src->Format == \
        HAL_PIXEL_FORMAT_RGB_565 ?  RK_FORMAT_RGB_565 : RK_FORMAT_RGBX_8888;
   ...
   // ALOGD("RenderToWfd use time=%ld ms",usec1);
    return 0;
...
static hwc_display_t hw_fb_dpy = NULL;
static hwc_surface_t hw_fb_sur = NULL;

void SurfaceFlinger::doComposition() {
    ATRACE_CALL();
    const bool repaintEverything = android_atomic_and(0, &mRepaintEverything);
    Target_info wfd_info = {0,};
    Target_info fb_info = {0,};
    HWComposer& hwc(getHwComposer());
    bool hasLcdComposition = false;
    size_t dpy_wfd = 0;
    hwc_display_t hw_wfd_dpy;
    hwc_surface_t hw_wfd_sur;

#if RENDER_WFD_FROM_FB
    for (size_t dpy_t=0 ; dpy_t<mDisplays.size() ; dpy_t++)
    {
        const sp<DisplayDevice>& hw_t(mDisplays[dpy_t]);

        if(!strcmp(hw_t->getDisplayName().string(),"Built-in Screen"))
        {
            const int32_t id = hw_t->getHwcDisplayId();
            if (id >= 0)
            {
                hasLcdComposition = hwc.hasLcdComposition(id);
            }
        }
    }
#endif
    for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
        const sp<DisplayDevice>& hw(mDisplays[dpy]);
        hwc_display_t hw_dpy;
        hwc_surface_t hw_sur;

        #if RENDER_WFD_FROM_FB
        if(!strcmp(hw->getDisplayName().string(),"Built-in Screen")
           && hw_fb_dpy == NULL)
        {
            hw_fb_dpy = eglGetCurrentDisplay();
            hw_fb_sur = eglGetCurrentSurface(EGL_DRAW);
        }

        // inform the h/w that we're done compositing

        if(!hasLcdComposition && strcmp(hw->getDisplayName().string(),"Built-in Screen"))
        {
            #ifdef TARGET_RK30
            ...
            dpy_wfd = dpy;
            DisplayDevice::makeCurrent(mEGLDisplay, hw, mEGLContext);
            hw_wfd_dpy = hw_dpy = eglGetCurrentDisplay();
            hw_wfd_sur = hw_sur = eglGetCurrentSurface(EGL_DRAW);
            ...
            if (_eglGetRenderBufferANDROID == NULL || _eglRenderBufferModifiedANDROID == NULL)
            {
            ...
            else
            {
                 backBuffer = (android_native_buffer_t *)
                        _eglGetRenderBufferANDROID((EGLDisplay) hw_dpy, (EGLSurface) hw_sur);

            }
            if (backBuffer == NULL)
            {
                ALOGE("%s(%d):Get back buffer Failed.", __FUNCTION__, __LINE__);
            }
            else
            {
                handle = (struct private_handle_t *) backBuffer->handle;

                if (handle == NULL)
                {
                    ALOGE("%s(%d):Get back buffer handle =NULL.", __FUNCTION__, __LINE__);
                }
                else
                {
                    #ifdef TARGET_BOARD_PLATFORM_RK30XXB
                        wfd_info.VAddr = handle->iBase;
                        wfd_info.Width = handle->iWidth;
                        wfd_info.Height = handle->iHeight;
                        wfd_info.Format =  handle->iFormat;
                    #else

                        wfd_info.VAddr = handle->base;
                        wfd_info.Width = handle->width;
                        wfd_info.Height = handle->height;
                        wfd_info.Format =  handle->format;
                    #endif
                }
            }
            if (_eglGetRenderBufferANDROID == NULL)
            {
                ALOGE("EGL_ANDROID_get_render_buffer extension 2");
            }
            else
            {
                 backBuffer = (android_native_buffer_t *)
                        _eglGetRenderBufferANDROID((EGLDisplay) hw_fb_dpy, (EGLSurface) hw_fb_sur);

            }
            if (backBuffer == NULL)
            {
                ALOGE("%s(%d):Get back buffer Failed,hw_fb_dpy=%p,hw_fb_sur=%p", __FUNCTION__, __LINE__,hw_fb_dpy,hw_fb_sur);
            }
            else
            {
                handle = (struct private_handle_t *) backBuffer->handle;
                ...
                {
                    #ifdef TARGET_BOARD_PLATFORM_RK30XXB
                        fb_info.VAddr = handle->iBase;
                        fb_info.Width = handle->iWidth;
                        fb_info.Height = handle->iHeight;
                        fb_info.Format =  handle->iFormat;
                    #else

                        fb_info.VAddr = handle->base;
                        fb_info.Width = handle->width;
                        fb_info.Height = handle->height;
                        fb_info.Format =  handle->format;
                    #endif
                }
            }
        }
...
#ifdef TARGET_RK30
    	{
    		...
    		if (mHadIndex > 0)
    		{
    			...
    			{
    				ALOGV("+++ restore the data addr=%p,width=%d,height=%d,i=%d",mHadBase[i],mFrameHead[i].FrameWidth,mFrameHead[i].FrameHeight,i);
    				const LayerVector& layers(mDrawingState.layersSortedByZ);
    				const size_t count = layers.size();
    				for (size_t j=0 ; j<count ; j++)
    				{
    				    if (mVideoLayers[i].get() == layers[j].get())
    				    {
    					   // if (mVideoLayers.size()>0 && mVideoLayers[i]->getBufferState(mHadBase[i]) != 0)
    					    {
    						    //ALOGD("#### restore the data,bufferstate=%d",mVideoLayers[i]->getBufferState(mHadBase[i]));
    ...
    postFramebuffer();
    #if RENDER_WFD_FROM_FB
    if(wfd_info.VAddr != 0 && fb_info.VAddr != 0)
    {
        const sp<DisplayDevice>& hw_t(mDisplays[dpy_wfd]);
        DisplayDevice::makeCurrent(mEGLDisplay, hw_t, mEGLContext);
        RenderToWfd(&wfd_info,&fb_info);
        #ifndef TARGET_BOARD_PLATFORM_RK30XXB
        _eglRenderBufferModifiedANDROID((EGLDisplay) hw_wfd_dpy, (EGLSurface) hw_wfd_sur);
        #endif
        hw_t->swapBuffers(getHwComposer());
    }
    #endif
                    ...
                    if (disp != NULL) {
                                ...
                                || (state.frame != draw[i].frame))
                        {
                            disp->setProjection(state.orientation,
                                    state.viewport, state.frame, mUseLcdcComposer);
                            if (mUseLcdcComposer && disp->getDisplayType() == DisplayDevice::DISPLAY_PRIMARY) {
                                for (size_t m=0 ; m<dc ; m++) {
                                    const ssize_t n = curr.indexOfKey(draw.keyAt(m));
                                    const DisplayDeviceState& stateExt(curr[n]);
                                    const wp<IBinder>& displayExt(curr.keyAt(n));
                                    const sp<DisplayDevice> dispExt(getDisplayDevice(displayExt));
                                    dispExt->setProjection(stateExt.orientation,
                                                stateExt.viewport, stateExt.frame, mUseLcdcComposer);
                                }
                            }
                        }
                    }
                ...
                if (draw.indexOfKey(curr.keyAt(i)) < 0) {
                    ...
                    if (stc != NULL) {
                        ...
                        sp<DisplayDevice> hw = new DisplayDevice(this,
                                ...
                                mEGLConfig, mHardwareOrientation);
                        hw->setLayerStack(state.layerStack);
                        hw->setProjection(state.orientation,
                                state.viewport, state.frame, false);
...
void SurfaceFlinger::computeVisibleRegions(
        ...
        Region& outDirtyRegion, Region& outOpaqueRegion, bool useLcdcComposer)
{
    ATRACE_CALL();
    ...
    while (i--) {
        ...
        if (useLcdcComposer && !strcmp("com.android.systemui.ImageWallpaper",layer->getName().string())) {
            Point lt;
            Rect tmp = visibleRegion.getBounds();
            lt.x = (tmp.left / 64) * 64;
            lt.y = tmp.top;
            tmp.setLeftTop(lt);
            visibleRegion.set(tmp);
        }
...
void SurfaceFlinger::doComposeSurfaces(const sp<const DisplayDevice>& hw, const Region& dirty)
{
    ...
    if (hasGlesComposition) {
        ...
        bool hasHwcComposition = hwc.hasHwcComposition(id);
        if(mAlphaEnabled) {
        	hasHwcComposition = true;
        	if(id == 0) {
        		char property[PROPERTY_VALUE_MAX];
				int scalevalue_x = 100, scalevalue_y = 100;
			    if (property_get("persist.sys.scalerate_x", property, "100") > 0) {
			    	scalevalue_x = atoi(property);
			    }
			    if (property_get("persist.sys.scalerate_y", property, "100") > 0) {
			    	scalevalue_y = atoi(property);
			    }
			    uint32_t xres, yres;
				...
				float xScale = (float)xres * scalevalue_x/(hw->getWidth() *100);
		        float yScale = (float)yres * scalevalue_y/(hw->getHeight()*100);
//				float xpos = (float)xres * (100 - scalevalue_x) / 200;
		        float ypos = (float)yres * (100 - scalevalue_y) / 200;

		        glTranslatef(0, 2 * ypos, 0.0f);
        ...
        if (hw->getDisplayType() >= DisplayDevice::DISPLAY_EXTERNAL) {
            ...
            if (!mUseLcdcComposer && scissor != bounds) {
    ...
    if (cur != end)
    {
        ...
        for (size_t i=0 ; i<count && cur!=end ; ++i, ++cur) {
            const sp<LayerBase>& layer(layers[i]);
            const Region clip(dirty.intersect(layer->mShouldTransform ?
                                  tr.transform(layer->visibleRegion):
                                  realTR.transform(layer->visibleRegion)));
            if (!clip.isEmpty()) {
...
#ifdef TARGET_RK30
                            srchnd = (struct private_handle_t *) cur->getHandle();
...
#ifndef TARGET_BOARD_PLATFORM_RK30XXB
        						   if (layer->getBufferState(srchnd->base) != 0)
#else
        						   if (layer->getBufferState(srchnd->iBase) != 0)
#endif
        		                  {
                      					if(mAlphaEnabled) {
                      						char property[PROPERTY_VALUE_MAX];
											hwc_layer_1_t* Layer = cur->gethwcLayer();
											hwc_rect_t rect = Layer->displayFrame;
											int left, right, top, bottom;
											if(property_get("sys.video.rect", property, NULL) > 0) {
												sscanf(property, "%d-%d-%d-%d", &left, &right, &top, &bottom);
											}
											else {
												left = 0;
												right = 0;
											...
											if(left != rect.left || right != rect.right || top != rect.top || bottom != rect.bottom) {
												memset(property, 0, PROPERTY_VALUE_MAX);
												sprintf(property, "%d-%d-%d-%d", rect.left, rect.right, rect.top, rect.bottom);
												property_set("sys.video.rect", property);
											}
                      						break;
                      					}
                      				        // render the layer
...
#endif
        							hw->RenderVPUBuffToLayerBuff(srchnd);
        							layer->draw(hw, clip);
#ifdef TARGET_BOARD_PLATFORM_RK30XXB
        ...
        for (size_t i=0 ; i<count ; ++i) {
            ...
            const Region clip(dirty.intersect(layer->mShouldTransform ?
                    ...
                    realTR.transform(layer->visibleRegion)));
            if (!clip.isEmpty()) {
                sp<Layer> ly(layer->getLayer());

               if( ly!= NULL && ly->mActiveBuffer != NULL)
               {
                        // ALOGD(" handle=%xp",ly->mActiveBuffer->handle);
                    srchnd = (struct private_handle_t *) ly->mActiveBuffer->handle;
               }
#ifdef TARGET_RK30
...
#endif
    					{
    						...
    						//struct tVPU_FRAME FrameHead;
    							...
    							//glFinish();
    							//glFlush();
    							//memcpy((void*)srchnd->base,&mFrameHead[mHadIndex],sizeof(tVPU_FRAME));
#ifdef TARGET_BOARD_PLATFORM_RK30XXB
    							mHadBase[mHadIndex] = (void*)srchnd->iBase;
#else
    							mHadBase[mHadIndex] = (void*)srchnd->base;
        ...
        uint32_t flags)
{
    ...
    sp<const DisplayDevice> hw(getDefaultDisplayDevice());
    //ALOGD("createLayer for (%d x %d), name=%s, ori=%d", w, h, name.string(), hw->getOrientation());
    switch (flags & ISurfaceComposerClient::eFXSurfaceMask) {
        case ISurfaceComposerClient::eFXSurfaceNormal: {
                if (mUseLcdcComposer &&
                        ((flags & ISurfaceComposerClient::eFakeTransformation) &&
                               hw->getHardwareRotation() % 2 == 1)) {
                    int tmp = w;
                    w = h;
                    h = tmp;
                }
        ...
        String8& result, char* buffer, size_t SIZE) const
{
    ...
    snprintf(buffer, SIZE, "Boot finished: %s\n", (mBootFinished ? "true":"false"));
    result.append(buffer);
        ...
        GLuint* textureName, GLfloat* uOut, GLfloat* vOut)
{
    ...
    HWComposer& hwc(getHwComposer());
    const int32_t id = hw->getHwcDisplayId();
    int hwc_num = hwc.getLayerNum(id);
    HWComposer::LayerListIterator cur = hwc.begin(id);
    const HWComposer::LayerListIterator end = hwc.end(id);
	struct private_handle_t* srchnd = NULL;;
#endif
...
#ifdef TARGET_RK30
		if(( !(cur->getFlags() & HWC_SKIP_LAYER)) && (hwc.initCheck() == NO_ERROR))
		{
			srchnd = (struct private_handle_t *)  cur->getHandle();
#ifdef TARGET_BOARD_PLATFORM_RK30XXB
			if( srchnd && (srchnd->iFormat == HAL_PIXEL_FORMAT_YCrCb_NV12_VIDEO))
        ...
        for (size_t i=0 ; i<count ; ++i) {
            ...
            if (z >= minLayerZ && z <= maxLayerZ) {
                ...
                if(hwc.initCheck() == NO_ERROR)
                {
	    	        ...
	    	        {
			        	if( !(cur->getFlags() & HWC_SKIP_LAYER)
			        		&& !strcmp(cur->getLayerName(),layer->getName().string())
			        	   )
				        {
