...
void NuPlayer::setVideoSurfaceTexture(const sp<ISurfaceTexture> &surfaceTexture) {
    ...
    msg->post();
    if ((isBuffering == false) && (mPauseFlag == false)) {
...
void NuPlayer::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        ...
        case kWhatStart:
        {
            ...
            postScanSources();
			ALOGD("NuPlayer  postScanSources %lld wifidisplay_flag %d this %x",
				systemTime(SYSTEM_TIME_MONOTONIC) / 1000,wifidisplay_flag,this);
            break;
        }
            ...
            if (what == ACodec::kWhatFillThisBuffer) {
                ...
                if (audio) {
                    ...
                    if (mVideoDecoder == NULL &&
                        ...
                        flags = AUDIO_OUTPUT_FLAG_FAST;//AUDIO_OUTPUT_FLAG_NONE;
                    }
        ...
        case kWhatRendererNotify:
        {
            ...
            if (what == Renderer::kWhatEOS) {
				...
				if(isBuffering && (mPauseFlag == false))
				{
...
void NuPlayer::renderBuffer(bool audio, const sp<AMessage> &msg) {
	...
	if(wifidisplay_flag!=0)
	{
		...
		if(audio == 0)
			mRenderer->rendervideobuffer(audio, buffer, reply);
		else
...
#if 0
int  NuPlayer::Wifidisplay_get_TimeInfo(void* ptr,int64_t *start_time,int64_t *audio_start_time)
{
	NuPlayer* mNuPlayer = static_cast <NuPlayer *>(ptr);
	if(mNuPlayer->wifidisplay_flag!=0x1234)
	{
		return -1;
	}
	else
	{
		int ret = mNuPlayer->mSource->Wifidisplay_get_TimeInfo(start_time,audio_start_time);
		if(*start_time != 0 && *audio_start_time != 0)
			mNuPlayer->mRenderer->Wifidisplay_set_TimeInfo(*start_time,*audio_start_time);
		return ret;
}
}
extern "C" int Wifidisplay_get_Time(void* ptr,int64_t *start_time,int64_t *audio_start_time)
{

	return NuPlayer::Wifidisplay_get_TimeInfo( ptr,start_time,audio_start_time);
}
#endif
}  // namespace android
