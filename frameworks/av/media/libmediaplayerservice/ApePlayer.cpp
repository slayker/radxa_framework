...
ApePlayer::ApePlayer() :
    ...
    mStreamType(AUDIO_STREAM_MUSIC), mLoop(false), mAndroidLoop(false),
    mExit(false), mPaused(false), mRender(false), mRenderTid(-1),mApeDec(NULL),mLastTime(-1)
{
    ALOGD("constructor\n");
...
{
    ALOGE("setDataSource url=%s, fd=%d, offset=%d, length=%d\n", path, fd, (int)offset, (int)length);
	...
	if(path == NULL)
	{
		truepath = getpath(getpid(),fd);
		ALOGI("--------->get apetrue %s",truepath);
		mApeDec = CreateIAPEDecompress(truepath, &nRetVal,1);
		free(truepath);
...
status_t ApePlayer::seekTo(int position)
{
	...
	unsigned long lp = position;
	unsigned long ls = mApeDec->GetInfo(APE_INFO_SAMPLE_RATE);
	unsigned long  block = lp/1000*ls;
...
status_t ApePlayer::getCurrentPosition(int* position)
{
  ...
  //  mApeDec->ApeDecodeGetCurTime(position);
...
status_t ApePlayer::getDuration(int* duration)
{
   ...
   // mApeDec->ApeDecodeGetTotalTime(duration);
...
status_t ApePlayer::reset_nosync()
{
    ...
    if (mFile != NULL) {
        ALOGE("OOPS! Ape didn't close the file");
        fclose(mFile);
    }
    mState = STATE_ERROR;
...
/*
    ALOGE("Create AudioTrack object: rate=%ld, channels=%d\n",
            rate, channels);*/
...
//int align=mApeDec->GetInfo(APE_INFO_BLOCK_ALIGN);
int trueblock;
    ALOGD("render\n");
//int  maxbuffersize=align*blocksperframe; 
    ...
    while (1) 
	{
		...
		/*
			ALOGE("START CREATE TRACK\n");
			createOutputTrack();
			ALOGE("END  CREATE TRACK\n");	*/
            }
            if (mExit) break;
            ...
            // render ape data into the input buffer
          //  mApeDec->ApeDecodeDec(mAudioBuffer, &numread);
		...
		numread = trueblock * mApeDec->GetInfo(APE_INFO_BLOCK_ALIGN);
            ...
            if (numread == 0) {
                ...
                if (mLoop || mAndroidLoop) {
                    //ApeDecodeSeek(0);
                    //ApeDecodeDec(mAudioBuffer, &numread);
                } else {
                    mAudioSink->stop();
                    audioStarted = false;
                    mRender = false;
                    ...
                    ALOGE("send MEDIA_PLAYBACK_COMPLETE");
                    sendEvent(MEDIA_PLAYBACK_COMPLETE);

                    // wait until we're started again
                    ALOGE("playback complete - wait for signal");
                    mCondition.wait(mMutex);
                    ALOGE("playback complete - signal rx'd");
                    if (mExit) break;
