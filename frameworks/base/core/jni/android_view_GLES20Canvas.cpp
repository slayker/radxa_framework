...
// Setup
// ----------------------------------------------------------------------------
static bool android_view_GLES20Canvas_queryHWRenderEngine(JNIEnv* env, jobject clazz,
        OpenGLRenderer* renderer, jstring property) {
    jboolean result = true;
    if (property != NULL) {
        const char* propertyArray = env->GetStringUTFChars(property, NULL);
        result = renderer->queryHWRenderEngine(propertyArray);
        env->ReleaseStringUTFChars(property, propertyArray);
    }
    return result;
}
static void android_view_GLES20Canvas_setOrientation(JNIEnv* env, jobject clazz,
        OpenGLRenderer* renderer, jint orientation) {
...
static JNINativeMethod gMethods[] = {
    ...
    { "nSetOrientation",    "(II)V",           (void*) android_view_GLES20Canvas_setOrientation },
    { "nQueryHWEngine",     "(ILjava/lang/String;)Z",
                                               (void*) android_view_GLES20Canvas_queryHWRenderEngine },
