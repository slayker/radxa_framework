...
//$_rbox_$_modify_$_lijiehong_$_begin_$add by ljh to support remote ctrl of musicplayer
...
//$_rbox_$_modify_$_lijiehong_$_end_$
		...
		/*$_rbox_$_modify_$_zhangwen_20121122: Initialize the location of the keymouse for the first time */
...
/*$_rbox_$_modify_$_zhangwen_20121122: use left,righe,up,down to change the location of mouse pointer In Infrare mouse Mode  */
//$_rbox_$_modify_$_begin
			...
			public void handleMessage(Message msg){
				...
				try{
					mWindowManager.dispatchMouse(mdeltax,mdeltay,screenWidth,screenHeight);
						...
						// x = String.valueOf(mKeyX); 
						// y = String.valueOf(mKeyY); 
				if(keydown){
							mKeyMouseHandler.sendEmptyMessageDelayed(msg.what,30);
							//mKeyMouseHandler.sendEmptyMessage(msg.what);			
								}
							}
...
//$_rbox_$_modify_$_lijiehong_$_begin add support for TV Media Key
...
//$_rbox_$_modify_$_lijiehong_$_begin add by ljh to support remote control of musicplayer
...
//$_rbox_$_modify_$_lijiehong_$ end

		/*$_rbox_$_modify_$_zhangwen_20121122: use this Method to achieve the function of Infrared mouse*/
	   //$_rbox_$_modify_$_begin
		mstate=SystemProperties.get("sys.KeyMouse.mKeyMouseState");
		if(mstate.equals("on")&&((keyCode==230)||(keyCode==231)||(keyCode==232)||(keyCode==233)||(keyCode==234))){
				if(down) keydown=true;
				else  keydown=false;
			mKeyMouseHandler.sendEmptyMessage(keyCode);		
			//return -1;
		}
		if(mstate.equals("on")&&((keyCode==66)||(keyCode==23)))
	 	    return -1;
		//$_rbox_$_modify_$_end
    ...
    void setHdmiPlugged(boolean plugged) {
        if (mHdmiPlugged != plugged) {
			...
			/*
			* when HDMI plug, redraw Focus window
			*/
			try 
			{
	            mWindowManager.invalidateFocusWindow();
	        } 
			catch (RemoteException e) 
			{
	        }
