...
/*$_rbox_$_modify_$_zhangwen_20121122: make permission for property_get/property_set*/
//$_rbox_$_modify_$_begin
#include <cutils/properties.h>
//$_rbox_$_modify_$_end

//$_rbox_$_modify_$_chenxiao_$_begin add to support mouse wheel zoom
#define ZOOM_MIN_SCALE 5
#define ZOOM_MAX_SCALE 40
#define ZOOM_MIN_DISTANCE 30
#define ZOOM_MAX_DISTANCE 400
...
InputDevice* InputReader::createDeviceLocked(int32_t deviceId,
	...
	/*$_rbox_$_modify_$_zhangwen_20121122: Infrare mouse devices*/
	//$_rbox_$_modify_$_begin
...
/*$_rbox_$_modify_$_zhangwen_20121122: init the key ok*/
//$_rbox_$_modify_$_begin
...
void CursorButtonAccumulator::process(const RawEvent* rawEvent) {
    if (rawEvent->type == EV_KEY) {
        ...
        case BTN_LEFT:
            ...
            break;
        case BTN_RIGHT:
            //change mouse right btn as back key,lijiehong
            mBtnBack = rawEvent->value;//mBtnRight = rawEvent->value;
            break;
        case BTN_MIDDLE:
        ...
        case BTN_TASK:
            ...
            break;
		/*$_rbox_$_modify_$_zhangwen_20121122:  define the key ok for infrare mouse*/
		//$_rbox_$_modify_$_begin
		case 28:
            mBtnOk1 = rawEvent->value;
			break;
		case 232:
...
/*$_rbox_$_modify_$_zhangwen_20121122:  mark the key state for infrare mouse*/
//$_rbox_$_modify_$_begin
	char * mKeyMouseState=new char[5];
...
//$_rbox_$_modify_$_end
    if (down) {
/*$_rbox_$_modify_$_zhangwen_20121122:  the keyCode of 234 for switch mode between*/
//$_rbox_$_modify_$   Infrare mouse and Remote control,you can see the code in
//$_rbox_$_modify_$   frameworks/base/data/keyboards/Generic.kl:
//$_rbox_$_modify_$   key 388   TV_KEYMOUSE_MODE_SWITCH
//$_rbox_$_modify_$   notify:the key 388 is the code in remotecl control driver file,you can change it you need 
//$_rbox_$_modify_$_begin
		if(keyCode==234){
...
/*$_rbox_$_modify_$_zhangwen_20121122:In Infrare mouse mode,change the keyCode of left,right,up,down*/
//$_rbox_$_modify_$_begin
	if(strcmp(mKeyMouseState,"on")==0){
		if(keyCode==21)       keyCode=230;
		else if(keyCode==22)  keyCode=231;
		else if(keyCode==19)  keyCode=232;
		else if(keyCode==20)  keyCode=233;
		}
//$_rbox_$_modify_$_end
...
CursorInputMapper::CursorInputMapper(InputDevice* device) :
        InputMapper(device) {
	//$_rbox_$_modify_$_chenxiao_$add to support mouse wheel zoom
	mDisplayWidth = 1920;
       mDisplayHeight = 1080;
	//$_rbox_$_modify_$_chenxiao_$ end
}
...
void CursorInputMapper::configure(nsecs_t when,
    ...
    if (config->getDisplayInfo(false /*external*/, &displayViewport)) {
    	mDisplayWidth = displayViewport.deviceWidth;
    	mDisplayHeight = displayViewport.deviceHeight;
    } else {
	mDisplayWidth = 1920;
	mDisplayHeight = 1080;
    }
...
void CursorInputMapper::process(const RawEvent* rawEvent) {
    ...
    if (rawEvent->type == EV_SYN && rawEvent->code == SYN_REPORT) {
	  //$_rbox_$_modify_$_chenxiao_$ add to support mouse wheel zoom
        if(mCursorButtonAccumulator.getButtonState()&AMOTION_EVENT_BUTTON_TERTIARY){
            char *state = new char[8];
            property_get("persist.sys.enable_wheel_zoom",state,"false");
            if(strcmp(state,"true")==0){
                sync_zoom(rawEvent->when);
            }
            delete(state);
        }else
            sync(rawEvent->when);
...
void CursorInputMapper::sync_zoom(nsecs_t when) {
    ...
    //if (mParameters.orientationAware && mParameters.hasAssociatedDisplay >= 0
   ...
   // synthesizeButtonKeys(getContext(), AKEY_EVENT_ACTION_DOWN, when, getDeviceId(), AINPUT_SOURCE_TOUCHSCREEN,
    ...
    if (downChanged || /*moved ||*/ scrolled || buttonsChanged) {
        ...
        if (downChanged) {
            motionEventAction = down ? AMOTION_EVENT_ACTION_POINTER_DOWN : AMOTION_EVENT_ACTION_POINTER_UP;
    ...
    PointerProperties pointers[2];
    pointers[0] = pointers[1] = pointerProperties;
    pointers[1].id = 1;

    PointerCoords coords[2];
    coords[0] = coords[1] = pointerCoords;

    char * step_scale = new char[4];
    property_get("persist.sys.zoom_step_scale",step_scale,"10");
    int scale = atoi(step_scale);
    delete(step_scale);
    if(scale<ZOOM_MIN_SCALE) scale = ZOOM_MIN_SCALE;
    else if(scale >ZOOM_MAX_SCALE) scale = ZOOM_MAX_SCALE;
    mZoomLength += (scale * vscroll);

    char * init_spacing = new char[4];
    property_get("persist.sys.zoom_init_spacing",init_spacing,"100");
    int distance = atoi(init_spacing);
    delete(init_spacing);
    if(distance<ZOOM_MIN_DISTANCE) distance = ZOOM_MIN_DISTANCE;
    else if(distance >ZOOM_MAX_DISTANCE) distance = ZOOM_MAX_DISTANCE;

    float tmpx;
    if(x<(mDisplayWidth-distance))
        tmpx = x+distance+mZoomLength;
    else
        tmpx = mDisplayWidth;
    if(tmpx<x)tmpx = x+10;
    else if(tmpx>mDisplayWidth)tmpx = mDisplayWidth;
    coords[1].setAxisValue(AMOTION_EVENT_AXIS_X, tmpx);
    //float tmpy = y+mZoomLength+200;
    //if(tmpy<0)tmpy = 0;
    //else if(tmpy>720)tmpy = 720;
    //coords[1].setAxisValue(AMOTION_EVENT_AXIS_Y, tmpy);
    if(x>distance)
        tmpx = x-distance-mZoomLength;
    else
        tmpx = 0;
    if(tmpx<0)tmpx = 0;
    else if(tmpx>x)tmpx = x-10;
    coords[0].setAxisValue(AMOTION_EVENT_AXIS_X, tmpx);
    //tmpy = y-mZoomLength;
    //if(tmpy<0)tmpy = 0;
    //else if(tmpy>720)tmpy = 720;
    //coords[0].setAxisValue(AMOTION_EVENT_AXIS_Y, tmpy);
    mPointerController->setPresentation(PointerControllerInterface::PRESENTATION_SPOT);
    mPointerController->fade(PointerControllerInterface::TRANSITION_GRADUAL);
    uint32_t idToIndex[]={0,1};
    mPointerController->setButtonState(currentButtonState);
    mPointerController->setCursorSpots(coords,idToIndex,2);

    if(motionEventAction==AMOTION_EVENT_ACTION_POINTER_DOWN){
        motionEventAction = 0x105;
        NotifyMotionArgs args(when, getDeviceId(), AINPUT_SOURCE_TOUCHSCREEN, 0,
                AMOTION_EVENT_ACTION_DOWN, 0, metaState, currentButtonState, 0,
				displayId,
                1, &pointers[0], &coords[0], mXPrecision, mYPrecision, downTime);
        getListener()->notifyMotion(&args);
    }
        NotifyMotionArgs args2(when, getDeviceId(), AINPUT_SOURCE_TOUCHSCREEN, 0,
                motionEventAction, 0, metaState, currentButtonState, 0,
				displayId,
                2, pointers, coords, mXPrecision, mYPrecision, downTime);
        getListener()->notifyMotion(&args2);
    if(motionEventAction==AMOTION_EVENT_ACTION_POINTER_UP){
        NotifyMotionArgs args3(when, getDeviceId(), AINPUT_SOURCE_TOUCHSCREEN, 0,
                AMOTION_EVENT_ACTION_UP, 0, metaState, currentButtonState, 0,
				displayId,
                1, &pointers[1], &coords[1], mXPrecision, mYPrecision, downTime);
        getListener()->notifyMotion(&args3);
    }

        // Send hover move after UP to tell the application that the mouse is hovering now.
        /*if (motionEventAction == AMOTION_EVENT_ACTION_POINTER_UP
                && mPointerController != NULL) {
            NotifyMotionArgs hoverArgs(when, getDeviceId(), AINPUT_SOURCE_TOUCHSCREEN, policyFlags,
    ...
    //synthesizeButtonKeys(getContext(), AKEY_EVENT_ACTION_UP, when, getDeviceId(), AINPUT_SOURCE_TOUCHSCREEN,
    //        policyFlags, lastButtonState, currentButtonState);
...
/*$_rbox_$_modify_$_zhangwen_20121122:  add a new InputMapper,just for Infrare mouse*/
//$_rbox_$_modify_$_begin
KeyMouseInputMapper::KeyMouseInputMapper(InputDevice* device) :
        InputMapper(device) {
}
