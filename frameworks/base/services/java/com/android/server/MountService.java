...
/* $_rbox_$_modify_$_huangyonglin for adding the sdcard to share */					
                    if (ucb.path.equals(Environment.getSecondVolumeStorageDirectory().getPath())) {
                        ucb.handleFinished();
                    } else {
...
/* $_rbox_$_modify_$_huangyonglin: modified for the mass storage supporting*/
        final String path ;
		...
		if(volume ==null)
			{
				Slog.e(TAG, "updatePublicVolumeState volume =null###");
				return ;
			}
		else
			path = volume.getPath();
/* $_rbox_$_modify_$ end*/
		...
		if(path == null || !path.startsWith("/")) {
            Slog.w(TAG, String.format("Unknown volume state transition (%s -> %s) for %s",
                    state, state, path));
            return; 
        }
        synchronized (mVolumesLock) {
...
/* $_rbox_$_modify_$_huangyonglin for adding the sdcard to share */                    
                            if (Environment.getExternalStorageState().equals(Environment.MEDIA_SHARED) 
								&& Environment.getSecondVolumeStorageDirectory().getPath().equals(path)) {
...
/* $_rbox_$_modify_$ end */ 							
    ...
    private void notifyShareAvailabilityChange(final boolean avail) {
        ...
        for (int n = 0; n < mVolumes.size(); n++) {
            ...
            if (((getUmsEnabling())||(vs.equals(Environment.MEDIA_SHARED)))&&(!getUmsRecoverying())) { //if UMS is recoverying ,don't do this again
                m++;
            }
        }
/* $_rbox_$_modify_$_huangyonglin for adding the sdcard to share */
