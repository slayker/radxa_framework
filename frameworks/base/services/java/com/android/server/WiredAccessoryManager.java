...
final class WiredAccessoryManager implements WiredAccessoryCallbacks {
    ...
    private static final int BIT_USB_DEVICE_AUDIO_PLAYBACK = (1 << 5);
    private static final int BIT_USB_DEVICE_AUDIO_CAPTURE = (1 << 6);
	
    private static final int SUPPORTED_HEADSETS = (BIT_HEADSET|BIT_HEADSET_NO_MIC|
                                                   BIT_USB_HEADSET_ANLG|BIT_USB_HEADSET_DGTL|
                                                   BIT_HDMI_AUDIO|BIT_USB_DEVICE_AUDIO_PLAYBACK|
    ...
    class WiredAccessoryObserver extends UEventObserver {
                ...
                for (int i = 0; i < mUEventInfo.size(); ++i) {
                    ...
                    try {
                        ...
                        if (curState >= 30) {
							...
							if(device == AudioManager.DEVICE_OUT_USB_DEVICE)
						    	AudioCommon.setCurrentPlaybackDevice(Integer.toString(card));
							else
						    	AudioCommon.setCurrentCaptureDevice(Integer.toString(card));							
                            mContext.sendBroadcast(new Intent(AudioCommon.AUDIOPCMLISTUPDATE));							
                        }
						else if(curState > 0) {
        ...
        private List<UEventInfo> makeObservedUEventList() {
            ...
            uei = new UEventInfo(NAME_USB_AUDIO_PLAYBACK, BIT_USB_DEVICE_AUDIO_PLAYBACK, 0);
            if (uei.checkSwitchExists()) {
                retVal.add(uei);
            } else {
            ...
            uei = new UEventInfo(NAME_USB_AUDIO_CAPTURE, BIT_USB_DEVICE_AUDIO_CAPTURE, 0);
            if (uei.checkSwitchExists()) {
                retVal.add(uei);
            } else {
                ...
                synchronized (mLock) {
					...
					if(state>=30 || state<0){
                        ...
                        if(card>0){
							...
							if(device == AudioManager.DEVICE_OUT_USB_DEVICE)						
							    AudioCommon.setCurrentPlaybackDevice(AudioCommon.getLastSocPlayback());
							else
						    	AudioCommon.setCurrentCaptureDevice("0");																
						}
                        mContext.sendBroadcast(new Intent(AudioCommon.AUDIOPCMLISTUPDATE));							
					}
					else{
