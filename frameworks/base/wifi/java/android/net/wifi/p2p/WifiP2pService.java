...
public class WifiP2pService extends IWifiP2pManager.Stub {
    ...
    public WifiP2pService(Context context) {
		...
		// gwl
		//mP2pStateMachine.getHandler().post(new P2pScanRunnable());
    }
...
//gwl =================================
    private class P2pScanRunnable implements Runnable{
        ...
        @Override
        public void run() {
            if (DBG) Log.d(TAG,"-------------------> P2pScanRunnable : Enter");
			if (P2pConnected_Group == 0) {
            	if (DBG) Log.d(TAG,"------------------> handle the p2p_find CMD");
            	mP2pStateMachine.mWifiNative.p2pFind(60);
			}
            mP2pStateMachine.getHandler().postDelayed(new P2pScanRunnable(),60000);// 60s
        }
    }
//======================================
    ...
    class InactiveState extends State {
        ...
        @Override
        public boolean processMessage(Message message) {
            if (DBG) logd(getName() + "processMessage" + message.toString());
            switch (message.what) {
                case WifiP2pManager.CONNECT:
                ...
                case WifiMonitor.P2P_GROUP_STARTED_EVENT:
                   mGroup = (WifiP2pGroup) message.obj;
                   if (DBG) logd(getName() + " 1group started isgroupowner"+mGroup.isGroupOwner());
            ...
            switch (message.what) {
                ...
                case WifiMonitor.P2P_GROUP_STARTED_EVENT:
                    ...
                    if (mGroup.isGroupOwner()) {
                         ...
                         */
						logd("P2P_GROUP_STARTED_EVENT don't set mConnectedDevice when it is owner");//jmj
						...
						logd("P2P_GROUP_STARTED_EVENT set mConnectedDevice when it is client"+groupOwner.deviceName+groupOwner.deviceAddress);//jmj
            ...
            switch (message.what) {
                case WifiMonitor.AP_STA_CONNECTED_EVENT:
                    ...
                    if (deviceAddress != null) {
                        ...
                        if (DBG) logd(getName() + " ap sta connected mSavedProvDiscDevice"+mSavedProvDiscDevice);
						mConnectedDevice =device;
						logd(getName() + " ap sta connected device "+mConnectedDevice);
                        sendP2pPeersChangedBroadcast();
						// gwl p2p connected and is go
                ...
                case DhcpStateMachine.CMD_POST_DHCP_ACTION:
                    ...
                    break;
				case WifiP2pManager.CANCEL_CONNECT: //add by lt for test
				    if (DBG) logd(getName() + " cancel connect");
                case WifiP2pManager.REMOVE_GROUP:
